class OrdersController < ApplicationController
  ...
  def checkout
    ...
    if request.patch?
      ...
      @order.first_name = "arun"                                            if @order.first_name.blank?
      @order.last_name = "chavan"                                              if @order.last_name.blank?
      @order.company = "flipkart"                                                  if @order.company.blank?
      @order.email_address = "fish@example.com"                                          if @order.email_address.blank?
      @order.phone_number = "919191919191"                                if @order.phone_number.blank?
      @order.billing_address1 = "2 nd cross" + " " + " KR road"   if @order.billing_address1.blank?
      @order.billing_address3 = "Bangalore"                                                 if @order.billing_address3.blank?
      @order.billing_address4 = "India"                                             if @order.billing_address4.blank?
      @order.billing_postcode = "571301"                                                 if @order.billing_postcode.blank?
    end
  end
